...
public class VolumeAction<V> extends Action<V> {
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        if ( block.getType().equals(BlocklyConstants.ROB_ACTIONS_PLAY_SET_VOLUME) ) {
            ...
            return VolumeAction
                .make(VolumeAction.Mode.SET, helper.convertPhraseToExpr(expr), Jaxb2Ast.extractBlockProperties(block), Jaxb2Ast.extractComment(block));
        }
        NullConst<V> expr = NullConst.make(Jaxb2Ast.extractBlockProperties(block), Jaxb2Ast.extractComment(block));
        return VolumeAction
            .make(VolumeAction.Mode.GET, helper.convertPhraseToExpr(expr), Jaxb2Ast.extractBlockProperties(block), Jaxb2Ast.extractComment(block));
    }
