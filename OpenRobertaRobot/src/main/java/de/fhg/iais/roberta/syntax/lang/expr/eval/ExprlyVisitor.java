...
public class ExprlyVisitor<V> extends ExprlyBaseVisitor<Expr<V>> {
    ...
    public Expr<V> visitFunc(ExprlyParser.FuncContext ctx) throws UnsupportedOperationException {
        ...
        if ( f.equals("getRGB") ) {
            if ( args.size() == 3 ) {
                return (Expr<V>) new RgbColor<V>(BCMAKE, null, args.get(0), args.get(1), args.get(2), EmptyExpr.make(BlocklyType.NUMBER_INT));
            } else if ( args.size() == 4 ) {
                return (Expr<V>) new RgbColor<V>(BCMAKE, null, args.get(0), args.get(1), args.get(2), args.get(3));
            } else {
                Expr<V> empty = EmptyExpr.make(BlocklyType.NUMBER_INT);
                return (Expr<V>) new RgbColor<V>(BCMAKE, null, empty, empty, empty, empty);
            }
        }
        try {
